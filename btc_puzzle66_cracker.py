...
def genPrivKey(start,end,addr):
    global a
    for i in range(start, end+1):
        # formatted_input = str(int_to_bytes(i))
        a = hex(i)[2:]
        while len(a) < 64:
                if len(a) < 64:
                    a = '0' + a
...
if __name__ == "__main__":
    ...
    for i in range(4):  # Number of threads
        t = threading.Thread(target=randomGenPrivKey, args=(2**65,2**66-1,"13zb1hQbWVsc2S7ZTZnP2G4undNNpdh5so"))
        # t = threading.Thread(target=randomGenPrivKey, args=(1,1,"1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH"))
